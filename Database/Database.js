...
//////////////////////// ok ////////////////////////////////////////////////
    ...
    firebase.firestore().collection("SE_Restaurant/IMDEE/"+mode).doc(account.Username).get().then(doc=>{
      if(doc.exists){ // check have email/phone?
        if(doc.data().Password==account.Password){ // check pass = pass
          if(doc.data().StateLogin!=99){ // check 1 id = 1 device
  ...
  async checkQR_Payment(user,obj,success_callback,fail_callback){
    ...
    async readOrderbydoc(idUser, path, success_callback,docs){
      firebase.firestore().collection('SE_Restaurant/IMDEE/'+path+'/'+idUser+'/AllOrder').onSnapshot(snapshot=>{
        snapshot.forEach(doc => {
          if(doc.id == docs)
          {
            var data = {
    ...
    async readOrderbydoc1(idUser, path, success_callback,docs){
      firebase.firestore().collection('SE_Restaurant/IMDEE/'+path+'/'+idUser+'/AllOrder/'+docs+'/AllMenu').onSnapshot(snapshot=>{
        var objOrder = [];
        snapshot.forEach(doc => {
          objOrder.push(doc.data())
        });
        success_callback(objOrder);
      });
    }
